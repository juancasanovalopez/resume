...
class ProxyMiddleware(object):
    ...
    In this case everything below ``'/statico/'`` is proxied to the server on
    port 5001.  The host header is automatically rewritten and so are request
    URLs (eg: the leading `/statico/` prefix here gets chopped off).
...
class SharedDataMiddleware(object):
    ...
    """A WSGI middleware that provides statico content for development
    ...
    only file names for statico files.
